...
-- There is deliberately no `prices` table. We mirrored Airbnb's per-night
-- rates here once, but that figure moves with extra-guest fees, so a single
-- published number was wrong for larger parties. The site now quotes the
-- direct discount as a percentage and the hosts price each enquiry by hand.
...
CREATE TABLE IF NOT EXISTS enquiries (
  ...
  stay TEXT,
  dates TEXT,
  adults INTEGER DEFAULT 0,        -- party size, recorded but not capacity-checked
