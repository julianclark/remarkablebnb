...
// No price estimate is computed or sent. Airbnb's nightly rate varies with
// extra-guest fees, so any total we quoted from a mirrored per-night figure
// would be wrong for larger parties. The hosts quote the real price by hand
// when they reply, which is why the guest counts below matter.

// Guest counts are advisory: we record what was asked for and let the hosts
// judge it on reply, rather than blocking an over-capacity enquiry that might
// suit another room or the whole house.
function parseGuestCount(raw: string | undefined, max: number): number {
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const adults = parseGuestCount(data.adults, 20);
  const children = parseGuestCount(data.children, 20);
  ...
  try {
    const res = await env.DB.prepare(
      'INSERT INTO enquiries (name, email, stay, dates, adults, children, message) VALUES (?, ?, ?, ?, ?, ?, ?)'
    )
      .bind(name, email, stay, dates || null, adults, children, message)
      .run();
  ...
  const htmlBody = `
    ...
    <strong>Guests:</strong> ${escapeHtml(guestsLabel)}</p>
